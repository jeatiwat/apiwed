...
router.put("/:pid1/:pid2", async (req, res) => {
  ...
  conn.query(checkVoteSql1, [currentDate, id1], (err, voteCountResult1) => {
    if (err) {
    ...
    conn.query(checkVoteSql2, [currentDate, id2], (err, voteCountResult2) => {
      if (err) {
        console.error("Error:", err);
        res.status(500).json({ error: "Internal Server Error" });
        return;
      ...
      if (voteCountResult1[0].vote_count > 0) {
        ...
        conn.query(updateSql1, [vid1.vote, id1, currentDate], (err, result1) => {
          if (err) {
            console.error("Error:", err);
            res.status(500).json({ error: "Internal Server Error" });
            return;
          ...
          if (voteCountResult2[0].vote_count > 0) {
            ...
            conn.query(updateSql2, [vid2.vote, id2, currentDate], (err, result2) => {
              if (err) {
                console.error("Error:", err);
                res.status(500).json({ error: "Internal Server Error" });
                return;
            ...
            let insertSql2 = mysql.format("INSERT INTO `votes` (`vote`, `date`, `pid`) VALUES (?, ?, ?)", [vid2.vote, currentDate, id2]);
        ...
        let insertSql1 = mysql.format("INSERT INTO `votes` (`vote`, `date`, `pid`) VALUES (?, ?, ?)", [vid1.vote, currentDate, id1]);
